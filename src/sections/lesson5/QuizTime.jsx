...
function ScoreDisplay({ score, answers, onContinue }) {
  ...
  const getMessage = () => {
    if (percentage === 100) return "Perfect score! You've mastered the WHAT framework!";
    if (percentage >= 80) return "Excellent work! You're ready to plan your own tools!";
    if (percentage >= 60) return "Good job! You've got a solid grasp of the framework.";
    return "Keep learning! Review the WHAT framework and try again.";
  };
...
function QuizTime({ onComplete }) {
  const { setQuizAnswer } = useLesson5();
  ...
  const handleNext = () => {
    ...
    if (currentQuestion < quiz5Questions.length - 1) {
    ...
    <div className="max-w-3xl mx-auto px-6 py-12">
      ...
      >
        ...
        <p className="text-slate-400">
          Let's see what you learned about the WHAT framework!
        </p>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {quizComplete ? (
          ...
          >
            <QuestionCard
              question={quiz5Questions[currentQuestion]}
            ...
            {showFeedback && (
              ...
              >
                <Button variant="primary" onClick={handleNext}>
                  {currentQuestion < quiz5Questions.length - 1 ? 'Next Question' : 'See Results'}
                  <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              </motion.div>
            )}
