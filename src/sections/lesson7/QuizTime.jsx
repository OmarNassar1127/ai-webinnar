...
function ScoreDisplay({ score, answers, onContinue }) {
  ...
  const getMessage = () => {
    if (percentage === 100) return "Perfect score! You're a data & AI master!";
    if (percentage >= 80) return "Excellent work! You understand how AI can transform your data work!";
    if (percentage >= 60) return "Good job! You've got the basics down.";
    return "Keep learning! Review the data & AI concepts and try again.";
  };
...
function QuizTime({ onComplete }) {
  const { setQuizAnswer } = useLesson7();
  ...
  const handleNext = () => {
    ...
    if (currentQuestion < quiz7Questions.length - 1) {
    ...
    <div className="max-w-3xl mx-auto px-6 py-12">
      ...
      >
        ...
        <p className="text-slate-400">
          Let's see what you learned about Data & AI!
        </p>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {quizComplete ? (
          ...
          >
            <QuestionCard
              question={quiz7Questions[currentQuestion]}
            ...
            {showFeedback && (
              ...
              >
                <Button variant="primary" onClick={handleNext}>
                  {currentQuestion < quiz7Questions.length - 1 ? 'Next Question' : 'See Results'}
                  <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              </motion.div>
            )}
