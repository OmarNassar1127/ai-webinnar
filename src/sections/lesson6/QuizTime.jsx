...
function ScoreDisplay({ score, answers, onContinue }) {
  ...
  const getMessage = () => {
    if (percentage === 100) return "Perfect score! You're ready to build amazing tools!";
    if (percentage >= 80) return "Excellent work! You've got the specification process down!";
    if (percentage >= 60) return "Good job! You understand the key concepts.";
    return "Keep learning! Review the specification workshop and try again.";
  };
...
function QuizTime({ onComplete }) {
  const { setQuizAnswer } = useLesson6();
  ...
  const handleNext = () => {
    ...
    if (currentQuestion < quiz6Questions.length - 1) {
    ...
    <div className="max-w-3xl mx-auto px-6 py-12">
      ...
      >
        ...
        <p className="text-slate-400">
          Let's see what you learned about building for operations!
        </p>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {quizComplete ? (
          ...
          >
            <QuestionCard
              question={quiz6Questions[currentQuestion]}
            ...
            {showFeedback && (
              ...
              >
                <Button variant="primary" onClick={handleNext}>
                  {currentQuestion < quiz6Questions.length - 1 ? 'Next Question' : 'See Results'}
                  <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              </motion.div>
            )}
