...
const STORAGE_KEY = 'vloto-ai-academy-progress-lesson3';
const DEBOUNCE_MS = 500;
const LESSON_ID = 3;
const TOTAL_SECTIONS = 8;

const Lesson3Context = createContext(null);
...
export function Lesson3Provider({ children }) {
  const { user } = useAuth();
  ...
  return (
    <Lesson3Context.Provider value={value}>
      {children}
    </Lesson3Context.Provider>
  );
}

export function useLesson3() {
  const context = useContext(Lesson3Context);
  if (!context) {
    throw new Error('useLesson3 must be used within a Lesson3Provider');
  }
  return context;
}

export default Lesson3Context;
