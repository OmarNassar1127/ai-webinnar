...
const STORAGE_KEY = 'vloto-ai-academy-progress-lesson8';
const UNLOCK_KEY = 'vloto-ai-academy-lesson8-unlocked';
const DEBOUNCE_MS = 500;
const LESSON_ID = 8;
const TOTAL_SECTIONS = 8;

const Lesson8Context = createContext(null);
...
// Check if Lesson 8 is unlocked
export const isLesson8Unlocked = () => {
  try {
...
// Unlock Lesson 8 (can be called from admin or special code entry)
export const unlockLesson8 = () => {
  try {
...
export function Lesson8Provider({ children }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    setIsUnlocked(isLesson8Unlocked());
  }, []);
  ...
  return (
    <Lesson8Context.Provider value={value}>
      {children}
    </Lesson8Context.Provider>
  );
}

export function useLesson8() {
  const context = useContext(Lesson8Context);
  if (!context) {
    throw new Error('useLesson8 must be used within a Lesson8Provider');
  }
  return context;
}

export default Lesson8Context;
