...
const STORAGE_KEY = 'vloto-ai-academy-progress-lesson5';
const DEBOUNCE_MS = 500;
const LESSON_ID = 5;
const TOTAL_SECTIONS = 7;

const Lesson5Context = createContext(null);
...
export function Lesson5Provider({ children }) {
  const { user } = useAuth();
  ...
  return (
    <Lesson5Context.Provider value={value}>
      {children}
    </Lesson5Context.Provider>
  );
}

export function useLesson5() {
  const context = useContext(Lesson5Context);
  if (!context) {
    throw new Error('useLesson5 must be used within a Lesson5Provider');
  }
  return context;
}

export default Lesson5Context;
