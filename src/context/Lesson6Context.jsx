...
const STORAGE_KEY = 'vloto-ai-academy-progress-lesson6';
const DEBOUNCE_MS = 500;
const LESSON_ID = 6;
const TOTAL_SECTIONS = 8;

const Lesson6Context = createContext(null);
...
export function Lesson6Provider({ children }) {
  const { user } = useAuth();
  ...
  return (
    <Lesson6Context.Provider value={value}>
      {children}
    </Lesson6Context.Provider>
  );
}

export function useLesson6() {
  const context = useContext(Lesson6Context);
  if (!context) {
    throw new Error('useLesson6 must be used within a Lesson6Provider');
  }
  return context;
}

export default Lesson6Context;
