...
const STORAGE_KEY = 'vloto-ai-academy-progress-lesson9';
const DEBOUNCE_MS = 500;
const LESSON_ID = 9;
const TOTAL_SECTIONS = 6;

const Lesson9Context = createContext(null);
...
export function Lesson9Provider({ children }) {
  const { user } = useAuth();
  ...
  return (
    <Lesson9Context.Provider value={value}>
      {children}
    </Lesson9Context.Provider>
  );
}

export function useLesson9() {
  const context = useContext(Lesson9Context);
  if (!context) {
    throw new Error('useLesson9 must be used within a Lesson9Provider');
  }
  return context;
}

export default Lesson9Context;
