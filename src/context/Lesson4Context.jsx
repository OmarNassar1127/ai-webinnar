...
const STORAGE_KEY = 'vloto-ai-academy-progress-lesson4';
const DEBOUNCE_MS = 500;
const LESSON_ID = 4;
const TOTAL_SECTIONS = 7;

const Lesson4Context = createContext(null);
...
export function Lesson4Provider({ children }) {
  const { user } = useAuth();
  ...
  return (
    <Lesson4Context.Provider value={value}>
      {children}
    </Lesson4Context.Provider>
  );
}

export function useLesson4() {
  const context = useContext(Lesson4Context);
  if (!context) {
    throw new Error('useLesson4 must be used within a Lesson4Provider');
  }
  return context;
}

export default Lesson4Context;
