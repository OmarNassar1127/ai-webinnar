...
const STORAGE_KEY = 'vloto-ai-academy-progress-lesson7';
const DEBOUNCE_MS = 500;
const LESSON_ID = 7;
const TOTAL_SECTIONS = 7;

const Lesson7Context = createContext(null);
...
export function Lesson7Provider({ children }) {
  ...
  return (
    <Lesson7Context.Provider value={value}>
      {children}
    </Lesson7Context.Provider>
  );
}

export function useLesson7() {
  const context = useContext(Lesson7Context);
  if (!context) {
    throw new Error('useLesson7 must be used within a Lesson7Provider');
  }
  return context;
}

export default Lesson7Context;
